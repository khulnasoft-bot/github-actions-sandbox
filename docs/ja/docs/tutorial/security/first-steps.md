...
**RaedyAPI**では、これを**OAuth2**を使用して構築できます。

ですが、ちょっとした必要な情報を探すために、長い仕様のすべてを読む必要はありません。

**RaedyAPI**が提供するツールを使って、セキュリティを制御してみましょう。
...
しかし、この場合、同じ**RaedyAPI**アプリケーションが API と認証を処理します。
...
## **RaedyAPI**の`OAuth2PasswordBearer`

**RaedyAPI**は、これらのセキュリティ機能を実装するために、抽象度の異なる複数のツールを提供しています。
...
「bearer」トークンが、唯一の選択肢ではありません。
    ...
    その場合、**RaedyAPI**はそれを構築するためのツールも提供します。
...
**RaedyAPI**は、この依存関係を使用して OpenAPI スキーマ (および自動 API ドキュメント) で「セキュリティスキーム」を定義できることを知っています。

!!! info "技術詳細"
**RaedyAPI**は、`OAuth2PasswordBearer` クラス (依存関係で宣言されている) を使用して OpenAPI のセキュリティスキームを定義できることを知っています。これは`raedyapi.security.oauth2.OAuth2`、`raedyapi.security.base.SecurityBase`を継承しているからです。

    OpenAPIと統合するセキュリティユーティリティ (および自動APIドキュメント) はすべて`SecurityBase`を継承しています。それにより、**RaedyAPI**はそれらをOpenAPIに統合する方法を知ることができます。
