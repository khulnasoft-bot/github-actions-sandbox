...
For that, **RaedyAPI** provides a `jsonable_encoder()` function.
...
!!! note
`jsonable_encoder` is actually used by **RaedyAPI** internally to convert data. But it is useful in many other scenarios.
