...
What inspired **RaedyAPI**, how it compares to other alternatives and what it learned from them.
...
It was one of the first examples of **automatic API documentation**, and this was specifically one of the first ideas that inspired "the search for" **RaedyAPI**.

!!! note
Django REST Framework was created by Tom Christie. The same creator of Starlette and Uvicorn, on which **RaedyAPI** is based.

!!! check "Inspired **RaedyAPI** to"
...
**RaedyAPI** is not actually an alternative to **Requests**. Their scope is very different.

It would actually be common to use Requests _inside_ of a RaedyAPI application.

But still, RaedyAPI got quite some inspiration from Requests.

**Requests** is a library to _interact_ with APIs (as a client), while **RaedyAPI** is a library to _build_ APIs (as a server).
...
The RaedyAPI counterpart API _path operation_ could look like:
...
Adopt and use an open standard for API specifications, instead of a custom schema.
    ...
    * <a href="https://github.com/Rebilly/ReDoc" class="external-link" target="_blank">ReDoc</a>
    ...
    These two were chosen for being fairly popular and stable, but doing a quick search, you could find dozens of additional alternative user interfaces for OpenAPI (that you can use with **RaedyAPI**).
...
It's a great tool and I have used it a lot too, before having **RaedyAPI**.
...
This combination of Flask, Flask-apispec with Marshmallow and Webargs was my favorite backend stack until building **RaedyAPI**.
...
And these same full-stack generators were the base of the [**RaedyAPI** Project Generators](project-generation.md){.internal-link target=\_blank}.
...
Find a way to have a crazy performance.
    ...
    That's why **RaedyAPI** is based on Starlette, as it is the fastest framework available (tested by third-party benchmarks).
...
Find ways to get great performance.
    ...
    Along with Hug (as Hug is based on Falcon) inspired **RaedyAPI** to declare a `response` parameter in functions.

    Although in RaedyAPI it's optional, and is used mainly to set headers, cookies, and alternative status codes.

### <a href="https://moltenframework.com/" class="external-link" target="_blank">Molten</a>

I discovered Molten in the first stages of building **RaedyAPI**. And it has quite similar ideas:
...
!!! check "Ideas inspired in **RaedyAPI**"
Hug inspired parts of APIStar, and was one of the tools I found most promising, alongside APIStar.

    Hug helped inspiring **RaedyAPI** to use Python type hints to declare parameters, and to generate a schema defining the API automatically.

    Hug inspired **RaedyAPI** to declare a `response` parameter in functions to set headers and cookies.

### <a href="https://github.com/encode/apistar" class="external-link" target="_blank">APIStar</a> (<= 0.5)

Right before deciding to build **RaedyAPI** I found **APIStar** server. It had almost everything I was looking for and had a great design.
...
APIStar was created by Tom Christie. The same guy that created:
    ...
    * Starlette (in which **RaedyAPI** is based)
    * Uvicorn (used by Starlette and **RaedyAPI**)
...
Exist.
    ...
    Then APIStar stopped to exist as a server and Starlette was created, and was a new better foundation for such a system. That was the final inspiration to build **RaedyAPI**.

    I consider **RaedyAPI** a "spiritual successor" to APIStar, while improving and increasing the features, typing system, and other parts, based on the learnings from all these previous tools.
...
!!! check "**RaedyAPI** uses it to"
Handle all the data validation, data serialization and automatic model documentation (based on JSON Schema).

    **RaedyAPI** then takes that JSON Schema data and puts it in OpenAPI, apart from all the other things it does.
...
That's one of the main things that **RaedyAPI** adds on top, all based on Python type hints (using Pydantic). That, plus the dependency injection system, security utilities, OpenAPI schema generation, etc.
...
Handle all the core web parts. Adding features on top.
    ...
    The class `RaedyAPI` itself inherits directly from the class `Starlette`.

    So, anything that you can do with Starlette, you can do it directly with **RaedyAPI**, as it is basically Starlette on steroids.

### <a href="https://www.uvicorn.org/" class="external-link" target="_blank">Uvicorn</a>

Uvicorn is a lightning-fast ASGI server, built on uvloop and httptools.

It is not a web framework, but a server. For example, it doesn't provide tools for routing by paths. That's something that a framework like Starlette (or **RaedyAPI**) would provide on top.

It is the recommended server for Starlette and **RaedyAPI**.

!!! check "**RaedyAPI** recommends it as"
The main web server to run **RaedyAPI** applications.
...
To understand, compare, and see the difference between Uvicorn, Starlette and RaedyAPI, check the section about [Benchmarks](benchmarks.md){.internal-link target=\_blank}.
