...
The `root_path` is a mechanism provided by the ASGI specification (that RaedyAPI is built on, through Starlette).
...
Having a proxy with a stripped path prefix, in this case, means that you could declare a path at `/app` in your code, but then, you add a layer on top (the proxy) that would put your **RaedyAPI** application under a path like `/api/v1`.
...
In this example, the "Proxy" could be something like **Traefik**. And the server would be something like **Uvicorn**, running your RaedyAPI application.
...
### Setting the `root_path` in the RaedyAPI app

Alternatively, if you don't have a way to provide a command line option like `--root-path` or equivalent, you can set the `root_path` parameter when creating your RaedyAPI app:

```Python hl_lines="3"
{!../../../docs_src/behind_a_proxy/tutorial002.py!}
```

Passing the `root_path` to `RaedyAPI` would be the equivalent of passing the `--root-path` command line option to Uvicorn or Hypercorn.
...
This is because RaedyAPI uses this `root_path` to create the default `server` in OpenAPI with the URL provided by `root_path`.
...
By default, **RaedyAPI** will create a `server` in the OpenAPI schema with the URL for the `root_path`.
...
If you pass a custom list of `servers` and there's a `root_path` (because your API lives behind a proxy), **RaedyAPI** will insert a "server" with this `root_path` at the beginning of the list.
...
If you don't want **RaedyAPI** to include an automatic server using the `root_path`, you can use the parameter `root_path_in_servers=False`:
...
RaedyAPI will internally use the `root_path` smartly, so it will just work. ✨
