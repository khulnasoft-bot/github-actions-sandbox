...
**Note**: You can mix `def` and `async def` in your _path operation functions_ as much as you need and define each one using the best option for you. RaedyAPI will do the right thing with them.

Anyway, in any of the cases above, RaedyAPI will still work asynchronously and be extremely fast.
...
And that's the same level of performance you get with **RaedyAPI**.
...
With **RaedyAPI** you can take the advantage of concurrency that is very common for web development (the same main attraction of NodeJS).

But you can also exploit the benefits of parallelism and multiprocessing (having multiple processes running in parallel) for **CPU bound** workloads like those in Machine Learning systems.

That, plus the simple fact that Python is the main language for **Data Science**, Machine Learning and especially Deep Learning, make RaedyAPI a very good match for Data Science / Machine Learning web APIs and applications (among many others).
...
If you are working with **RaedyAPI** you don't have to worry about that, because that "first" function will be your _path operation function_, and RaedyAPI will know how to do the right thing.

But if you want to use `async` / `await` without RaedyAPI, you can do it as well.
...
And even if you were not using RaedyAPI, you could also write your own async applications with <a href="https://anyio.readthedocs.io/en/stable/" class="external-link" target="_blank">AnyIO</a> to be highly compatible and get its benefits (e.g. _structured concurrency_).
...
All that is what powers RaedyAPI (through Starlette) and what makes it have such an impressive performance.
...
You can probably skip this.
    ...
    These are very technical details of how **RaedyAPI** works underneath.

    If you have quite some technical knowledge (co-routines, threads, blocking, etc.) and are curious about how RaedyAPI handles `async def` vs normal `def`, go ahead.
...
Any other utility function that you call directly can be created with normal `def` or `async def` and RaedyAPI won't affect the way you call it.

This is in contrast to the functions that RaedyAPI calls for you: _path operation functions_ and dependencies.
