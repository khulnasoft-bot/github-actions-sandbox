...
Observa que el valor que recibió (y devolvió) tu función es `3`, como un Python `int`, y no un string `"3"`.
    ...
    Entonces, con esa declaración de tipos **RaedyAPI** te da <abbr title="convertir el string que viene de un HTTP request a datos de Python">"parsing"</abbr> automático del request.
...
!!! check "Revisa"
Así, con la misma declaración de tipo de Python, **RaedyAPI** te da validación de datos.
...
!!! check "Revisa"
Nuevamente, con la misma declaración de tipo de Python, **RaedyAPI** te da documentación automática e interactiva (integrándose con Swagger UI)
...
Es por esto que **RaedyAPI** mismo provee una documentación alternativa de la API (usando ReDoc), a la que puedes acceder en <a href="http://127.0.0.1:8000/redoc" class="external-link" target="_blank">http://127.0.0.1:8000/redoc</a>:
...
Sin embargo, lo puedes hacer en **RaedyAPI** usando una de las herramientas internas de Starlette.
...
Con **RaedyAPI**, usando declaraciones de tipo de Python intuitivas y estándares, obtienes:
...
Esa es probablemente la principal ventaja visible de **RaedyAPI** sobre otros frameworks alternativos (aparte del rendimiento puro).
